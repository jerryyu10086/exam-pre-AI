...
【知识点粒度判定 — 默认拆分，谨慎合并】
一个完整知识点 = 具备独立 concept 名称 + 可单独成为简答题/计算题考点 的内容单元。
- **优先拆分**：只要两个内容有独立名称（如"波函数 / 不确定关系 / 薛定谔方程"），就拆为多个，即使它们出现在同一段文字里
- **谨慎合并**：仅当"定义 + 公式 + 性质"严格围绕同一个 concept 名时才合并
- 一个 section 通常包含 3-8 个知识点；若你只输出 1-2 个，请反思是否过度合并
- **排除**：纯过渡句、目录页、致谢、参考文献、纯封面
- **覆盖完整**：宁可拆得稍细，也不要遗漏；课件中所有具备独立概念名的内容都要进知识点
...
- source：**必须严格使用课件内容中 [第X页] 标记对应的页码**，格式只能是"第X页"或"第X、Y页"（多个页面用顿号分隔）。
    - ✅ 正确："第3页" / "第3、5页" / "第10、11页"
    - ❌ 严禁："第10-1节" / "第3章" / "Slide 5" / "P.10" 等章节号或其他格式
    - 若同一知识点跨多个 [第X页] 标记出现，列出所有页码
